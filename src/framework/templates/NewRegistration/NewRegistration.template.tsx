...
export const NewRegistration: React.FC<NewRegistrationProps> = ({handleSubmit}) => {
    ...
    const [isAllReqDone, setIsAllReqDone] = useState<boolean[]>([false, false, false]);

    useEffect(() => {
        getClassesApiService()
            .then((response: any) => {
                const FORMATTED_DATA = response.data.map((it: any) => {
                    return {label: it.descricao, value: it.id}
                });
                setClasses([{label: 'Selecione a turma', value: ''}, ...FORMATTED_DATA]);
                setIsAllReqDone(prev => [true, prev[1], prev[2]]);
            }).catch(error => console.error(error));

        getAllStudentsApiService()
            .then((response: any) => {
                setStudents(response.data.map((it: any) => {
                    return {label: it.nome, value: it.id}
                }));
                setIsAllReqDone(prev => [prev[0], true, prev[2]]);
            }).catch(err => console.error(err));

        getActivitiesApiService()
            .then((response: any) => {
                setActivities(response.data.filter((it: any) => !!it?.ativo));
                setIsAllReqDone(prev => [prev[0], prev[1], true]);
            }).catch(err => console.error(err));
    }, []);
        ...
        <S.Container>
            ...
            {!loading ? (
                <S.Body>
                    <S.InputSection>
                        <S.InputContainer>
                            <SelectInLabel label="Ano *" options={yearOptions} onChange={(v: any) => setAno(v.value)}
                                           selectedValue={ano}/>
                            <SelectInLabel label="Turma *" options={classes} onChange={(v: any) => setTurma(v.value)}
                                           selectedValue={turma}/>
                        </S.InputContainer>
                        <S.InputContainer>
                            <div style={{"width": "100%", "marginTop": "4px"}}>
                                <SearchSelect label="Pesquisar aluno *" options={students}
                                              onChange={(v: any) => setAluno(v)} value={aluno}/>
                            </div>
                        ...
                        <S.InputContainer>
                            <SelectInLabel label="Autoriza divulgação de dados pessoais"
                                           selectedValue={divulgacaoDadosAutorizada}
                                           onChange={(v: any) => setDivulgacaoDadosAutorizada(v.value)}
                                           options={simOuNaoOptions}/>
                            <SelectInLabel label="Autoriza a divulgação de sua imagem nas redes sociais da escola"
                                           selectedValue={divulgacaoImagemRede}
                                           onChange={(v: any) => setDivulgacaoImagemRede(v.value)}
                                           options={simOuNaoOptions}/>
                        </S.InputContainer>
                        <S.InputContainer>
                            <SelectInLabel label="Almoço" selectedValue={optouAlmoco}
                                           onChange={(v: any) => setOptouAlmoco(v.value)}
                                           options={simOuNaoOptions}/>
                            <SelectInLabel label="Jantar" selectedValue={optouJantar}
                                           onChange={(v: any) => setOptouJantar(v.value)}
                                           options={simOuNaoOptions}/>
                        </S.InputContainer>
                        <S.InputContainer>
                        ...
                        <S.InputContainer>
                            ...
                            <InputInLabel label="Valor material pedagógico" value={valorMaterialPedagogico}
                                          placeholder="R$"
                                          onChange={setValorMaterialPedagogico} type="currency"/>
                            <InputInLabel label="Valor projeto nutricional" value={valorProjetoNutricional}
                                          placeholder="R$"
                        ...
                        <S.InputContainer>
                            ...
                            <SelectInLabel label="Forma de pagamento *" options={formaPagamentoOptions}
                                           selectedValue={formaPagamentoParcelas}
                                           onChange={(v: any) => setFormaPagamentoParcelas(v.value)}/>
                        </S.InputContainer>
                        <S.GridContainer>
