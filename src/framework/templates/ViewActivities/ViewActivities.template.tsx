...
export const ViewActivities: React.FC<ViewActivitiesProps> = ({
    ...
    return (
        <S.Container>
            ...
            <Header title="Atividades" buttonText="Nova Atividade"
                    onButtonClick={() => navigate('/gestao-escolar/nova-atividade')}/>
            {!loading ? (
                <React.Fragment>
                    <S.FindClassContainer>
                        <Title size={20}>Encontre a atividade</Title>
                        <S.FilterContainer>
                            <InputInLabel label="Atividade" value={filters.descricao} placeholder="Digite aqui..."
                                          onChange={v => handleFilterChange('descricao', v)}/>
                            <SelectInLabel label="Situação" options={statusData} selectedValue={filters.situacao}
